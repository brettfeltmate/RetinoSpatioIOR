...
>> RetinoSpatioIOR_ind_vars.add_variable("color", str)
>> RetinoSpatioIOR_ind_vars.add_variable("size", float)
>> RetinoSpatioIOR_ind_vars.add_variable("active", bool)
>> RetinoSpatioIOR_ind_vars.add_variable("count", int)

Finally, we add values to each variable. This can be done one at a time, as in the colors example below:

>> RetinoSpatioIOR_ind_vars['color'].add_value("blue")
>> RetinoSpatioIOR_ind_vars['color'].add_value("blue")
>> RetinoSpatioIOR_ind_vars['color'].add_value("blue")

Or altogether in a comma-separated set, as in the 'count' example:

>> RetinoSpatioIOR_ind_vars['count'].add_values(1,2,3,4,5)
...
>> RetinoSpatioIOR_ind_vars['size'].add_value(1.0, 2)
>> RetinoSpatioIOR_ind_vars['size'].add_value(2.0, 2)
>> RetinoSpatioIOR_ind_vars['size'].add_value(5.0)
...
>> RetinoSpatioIOR_ind_vars['active'].add_values((True, 5), False)

which is equivalent to:

>> RetinoSpatioIOR_ind_vars['active'].add_value(True, 5)
>> RetinoSpatioIOR_ind_vars['active'].add_value(False)
...
>> RetinoSpatioIOR_ind_vars['size'].enabled = False
...
>> RetinoSpatioIOR_ind_vars['size'][1.0].enabled = False
